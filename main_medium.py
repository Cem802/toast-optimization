...
def find_maximum():
    best_params = (random.randint(1, 100), random.randint(1, 100))
    ...
    while True:
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if (best_params[0] + i * step_size < 1
                   or best_params[0] + i * step_size > 100
                   or best_params[1] + j * step_size < 1
                   or best_params[1] + j * step_size > 100
                   or (i == 0 and j == 0)
                ):
                    continue
                # calculate the utility of the neighbor
                new_params = (best_params[0] + i * step_size, best_params[1] + j * step_size)
                new_utility = utility(*new_params)
